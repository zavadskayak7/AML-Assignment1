...
plt.figure(1)
#plt.subplot(1,3,1) # There seems to be a bug, plt.subplot shows different histograms than the real ones in this case,thats why we removed the subplot command
plt.imshow(img_color)
plt.show()

print('There seems to be a bug, plt.subplot shows different histograms than the real ones in this case,thats why we removed the subplot command')
...
#!!!We have slightly transformed this part, so that we perform a grid search over all the possible combinations#####

print('for this part, we have slightly transformed the code,in order to perform a grid search over all combinations of hist and dist types')
...
nums_of_bins=[5,15,30] 
number_of_samples=30 # number of queries in the sample
query_images_sample=random.sample(query_images,number_of_samples)


for dist_type in tqdm(dist_types):
    for hist_type in tqdm(hist_types):
        for num_bins in tqdm(nums_of_bins):
            [best_match, D] = match_module.find_best_match(model_images, query_images_sample, dist_type, hist_type, num_bins)
            num_correct= sum([best_match[i]==int(query_images_sample[i].split('/')[1].split('_')[0].split('obj')[1]) for i in range(len(best_match))])
            print('settings : dist type=%s, hist type=%s, num of bins=%d,  number of correct matches= %d (%f)\n'% (dist_type,hist_type,num_bins,num_correct, 1.0 * num_correct / len(query_images_sample)))
